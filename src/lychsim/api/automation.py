# https://github.com/unrealcv/unrealcv/blob/5.2/client/python/unrealcv/automation.py
...
try:
    input = raw_input  # to support python3
except NameError:
    pass
...
class UE4Automation:
    ...
    def build_plugin(self, plugin_descriptor, output_folder, overwrite=False):
        ...
        if overwrite == False and os.path.isdir(abs_output_folder):
            print(
                'Output folder "%s" already exists, skip compilation.'
                % abs_output_folder
            )
            print(
                "Remove this folder if you want to compile the plugin with a different UE4 version."
            )
        else:
            ...
            subprocess.call(
                [
                    ...
                    "-rocket",
                    "-targetplatforms=%s" % self.platform_name,
                ],
                cwd=script_dir,
            )
    ...
    def install(self, plugin_folder, overwrite=False):
        ...
        print(
            "Copy the plugin from %s to %s"
            % (abs_src_unrealcv_folder, abs_tgt_unrealcv_folder)
        )
    ...
    def _get_UE4_dir(self):
        ...
        search_candidates = {
            ...
            "Mac": mac_candidates,
            "Win64": win_candidates,
        }
        candidates = search_candidates.get(self._get_platform_name())

        found_UE4 = []
        for c in candidates:
            found_UE4 += glob.glob(c)
        # Ask user to make a selection
        if len(found_UE4) == 1:
            return found_UE4[0]
        if len(found_UE4) == 0:
            print(
                "Can not automatically found a UE4 path, please specify it with --UE4"
            )
...
class LinuxBinary(UE4BinaryBase):
    def start(self):
        ...
        popen_obj = subprocess.Popen(
            [self.binary_path], stdout=null_file, stderr=null_file
        )
...
class MacBinary(UE4BinaryBase):
    def start(self):
        popen_obj = subprocess.Popen(["open", self.binary_path])
