...
class UnrealCv_API(object):
    ...
    def is_cam_pose_invalid(self, cam_id, pose, radius_cm=5.0):
        """
        Check whether a target camera pose location is invalid by overlap testing.

        Args:
            cam_id (int): The camera ID.
            pose (list): The camera pose [x, y, z, roll, yaw, pitch] or [x, y, z].
            radius_cm (float): Safety sphere radius in centimeters. Default is 5.0.

        Returns:
            bool: True if target location is invalid (collision/inside), else False.
        """
        if len(pose) < 3:
            raise ValueError(f"pose must include at least [x, y, z], got {pose}")
        x, y, z = pose[0], pose[1], pose[2]
        cmd = f"lych cam is_pose_invalid {cam_id} {x} {y} {z} {float(radius_cm)}"
        res = str(self.client.request(cmd)).strip().lower()
        if res in ("true", "1"):
            return True
        if res in ("false", "0"):
            return False
        raise ValueError(f"Failed to parse response for camera {cam_id}: {res}")
