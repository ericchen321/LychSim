...
public:
	...
	FStaticAnnotationSceneProxy(UStaticMeshComponent* Component, bool bForceLODsShareStaticLighting, UMaterialInterface* AnnotationMID, const UAnnotationComponent* AnnotationComponentRef) :
		FStaticMeshSceneProxy(Component, bForceLODsShareStaticLighting)
	{
		...
		bCastShadow = false;
		AnnotationComponent = AnnotationComponentRef;
	}
...
private:
	TWeakObjectPtr<const UAnnotationComponent> AnnotationComponent;
};
...
{
	if (AnnotationComponent.IsValid())
	{
		const UAnnotationComponent* AnnotationComp = AnnotationComponent.Get();
		if (AnnotationComp && !AnnotationComp->AllowsElement(ElementIndex))
		{
			return false;
		}
	}
...
void UAnnotationComponent::SetAllowedElement(int32 ElementIndex)
{
	AllowedElements.Reset();
	if (ElementIndex >= 0)
	{
		AllowedElements.Add(ElementIndex);
	}
}

void UAnnotationComponent::SetAllowedElements(const TArray<int32>& InElements)
{
	AllowedElements = InElements;
}

void UAnnotationComponent::ClearAllowedElements()
{
	AllowedElements.Reset();
}

bool UAnnotationComponent::AllowsElement(int32 ElementIndex) const
{
	return AllowedElements.Num() == 0 || AllowedElements.Contains(ElementIndex);
}
...
FPrimitiveSceneProxy* UAnnotationComponent::CreateSceneProxy(UStaticMeshComponent* StaticMeshComponent)
{
	...
	FPrimitiveSceneProxy* Proxy = ::new FStaticAnnotationSceneProxy(StaticMeshComponent, false, ProxyMaterial, this);
	return Proxy;
