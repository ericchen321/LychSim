...
void ADataCaptureActor::CaptureImageFromSensor(FString SensorName, UFusionCamSensor* Sensor)
{
	...
	if (bCaptureImage)
	{
		int Width = 0;
		int Height = 0;
		int WarmupFrames = 0; // deterministic warmup before capture
		TArray<FColor> LitData;
		if (bLitSlow)
		{
			Sensor->GetLit(LitData, Width, Height, WarmupFrames, ELitMode::Slow);
		}
		else
		{
			Sensor->GetLit(LitData, Width, Height, WarmupFrames, ELitMode::Lit);
		}
