...
TArray<uint8> FSerializationUtils::Array2Npy(const TArray<FColor>& ImageData, int32 Width, int32 Height, int32 Channel)
{
	...
	const unsigned char* Bytes = reinterpret_cast<const unsigned char*>(ImageData.GetData());
	const size_t NumBytes = static_cast<size_t>(Width) * static_cast<size_t>(Height) * 4;

	std::vector<unsigned char> NpyData(Bytes, Bytes + NumBytes);

	NpyHeader.insert(NpyHeader.end(), NpyData.begin(), NpyData.end());

	BinaryData.Reserve((int32)NpyHeader.size());
	for (char Element : NpyHeader)
	{
		BinaryData.Add((uint8)Element);
	}

	return BinaryData;
}

TArray<uint8> FSerializationUtils::Array2Npy4D(const TArray<TArray<FColor>>& ImageData, int32 Time, int32 Width, int32 Height, int32 Channel)
{
	TArray<uint8> BinaryData;
	...
	const int32 PixelsPerImage = Width * Height;
	for (int32 i = 0; i < Time; ++i)
	{
		if (ImageData[i].Num() != PixelsPerImage)
		{
			UE_LOG(LogUnrealCV, Warning, TEXT("Array2NpyU8Batch shape mismatch at %d"), i);
			return BinaryData;
		}
	}

	const size_t BytesPerImage = static_cast<size_t>(PixelsPerImage) * 4;
	std::vector<unsigned char> NpyData;
	NpyData.reserve(static_cast<size_t>(Time) * BytesPerImage);

	for (int32 i = 0; i < Time; ++i)
	{
		const unsigned char* Bytes = reinterpret_cast<const unsigned char*>(ImageData[i].GetData());
		NpyData.insert(NpyData.end(), Bytes, Bytes + BytesPerImage);
	}
