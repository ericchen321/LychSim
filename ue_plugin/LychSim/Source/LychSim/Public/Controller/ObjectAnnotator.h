...
public:
	...
	/** Annotate meshes by their individual material elements (part segmentation). */
	void AnnotateWorldByElement(UWorld* World);
...
private:
	...
	UAnnotationComponent* CreateAnnotationComponentForMesh(UMeshComponent* MeshComponent, const FColor& AnnotationColor, int32 ElementIndex = INDEX_NONE);
	FColor GetPartColor(const FString& PartId);
	FString BuildPartId(const FString& ActorName, const FString& ComponentName, int32 ElementIndex) const;
...
private:
	...
	TMap<FString, FColor> PartAnnotationColors; // Store annotation data (ActorName_Component_Element->Color)
};
